...
// WalksUsecase 散歩記録に関するビジネスロジックを提供するユースケース
...
// walksUsecaseImpl WalksUsecaseの実装
type walksUsecaseImpl struct {
	walksRepo repository.WalksRepository
}

// NewWalksUsecase WalksUsecaseの新しいインスタンスを作成
func NewWalksUsecase(walksRepo repository.WalksRepository) WalksUsecase {
...
func (u *walksUsecaseImpl) CreateWalk(ctx context.Context, req *model.CreateWalkRequest) (*model.CreateWalkResponse, error) {
	// 入力バリデーション
	if err := u.validateCreateWalkRequest(req); err != nil {
	...
	// エリア名を推定
	areaName := u.estimateAreaName(req.StartLocation)
	...
	walk := &model.Walk{
		...
		Theme:           req.Theme,
		POIIds:          poiIDs,
		Tags:            u.extractTagsFromTheme(req.Theme), // テーマからタグを生成
...
// GetWalksByBoundingBox 境界ボックス内の散歩記録一覧を取得
func (u *walksUsecaseImpl) GetWalksByBoundingBox(ctx context.Context, minLng, minLat, maxLng, maxLat float64) ([]model.WalkSummary, error) {
	// 境界ボックスのバリデーション
	if err := u.validateBoundingBox(minLng, minLat, maxLng, maxLat); err != nil {
		return nil, fmt.Errorf("境界ボックスの検証失敗: %w", err)
	}

	// リポジトリから散歩記録を取得（高速境界ボックス検索）
	walks, err := u.walksRepo.GetWalksByBoundingBox(ctx, minLng, minLat, maxLng, maxLat)
	if err != nil {
...
func (u *walksUsecaseImpl) validateCreateWalkRequest(req *model.CreateWalkRequest) error {
	if req.Title == "" {
...
func (u *walksUsecaseImpl) validateBoundingBox(minLng, minLat, maxLng, maxLat float64) error {
	if minLng >= maxLng {
...
func (u *walksUsecaseImpl) estimateAreaName(location *model.Location) string {
...
func (u *walksUsecaseImpl) extractTagsFromTheme(theme string) []string {
