...
// HistoryStrategy は歴史的スポットを巡るルートを提案する
type HistoryStrategy struct{}

func NewHistoryStrategy() StrategyInterface {
	return &HistoryStrategy{}
}

// GetAvailableScenarios はHistoryテーマで利用可能なシナリオ一覧を取得する
func (s *HistoryStrategy) GetAvailableScenarios() []string {
	return model.GetHistoryScenarios()
}

// GetTargetCategories は指定されたシナリオで検索対象となるPOIカテゴリを取得する
func (s *HistoryStrategy) GetTargetCategories(scenario string) []string {
	switch scenario {
...
func (s *HistoryStrategy) FindCombinations(scenario string, candidates []*model.POI) [][]*model.POI {
...
func (s *HistoryStrategy) findTempleShrineComnations(spots []*model.POI) [][]*model.POI {
	// 神社仏閣を優先的に選択
...
func (s *HistoryStrategy) findMuseumTourCombinations(spots []*model.POI) [][]*model.POI {
	// 博物館・美術館を中心とした組み合わせ
...
func (s *HistoryStrategy) findOldTownCombinations(spots []*model.POI) [][]*model.POI {
...
func (s *HistoryStrategy) findCulturalWalkCombinations(spots []*model.POI) [][]*model.POI {
...
func (s *HistoryStrategy) findDefaultCombinations(spots []*model.POI) [][]*model.POI {
	spotA := helper.FindHighestRated(spots)
	others := helper.RemovePOI(spots, spotA)
	helper.SortByDistance(spotA, others)
...
func (s *HistoryStrategy) FindCombinationsWithDestination(scenario string, destinationPOI *model.POI, candidates []*model.POI) [][]*model.POI {
	targetCategories := s.GetTargetCategories(scenario)
	filteredSpots := helper.FilterByCategory(candidates, targetCategories)
...
func (s *HistoryStrategy) findTempleShrineComnationsWithDestination(destination *model.POI, spots []*model.POI) [][]*model.POI {
	// 神社仏閣を優先的に選択
	temples := helper.FilterByCategory(spots, []string{"place_of_worship"})
...
func (s *HistoryStrategy) findMuseumTourCombinationsWithDestination(destination *model.POI, spots []*model.POI) [][]*model.POI {
	// 博物館・美術館を優先
	museums := helper.FilterByCategory(spots, []string{"museum", "art_gallery"})
...
func (s *HistoryStrategy) findOldTownCombinationsWithDestination(destination *model.POI, spots []*model.POI) [][]*model.POI {
	// 観光地を中心とした選択
	attractions := helper.FilterByCategory(spots, []string{"tourist_attraction"})
...
func (s *HistoryStrategy) findCulturalWalkCombinationsWithDestination(destination *model.POI, spots []*model.POI) [][]*model.POI {
	// バランス良く文化的スポットを選択
...
func (s *HistoryStrategy) findDefaultCombinationsWithDestination(destination *model.POI, spots []*model.POI) [][]*model.POI {
	// 評価の高い順にソートして2つ選択
	helper.SortByRating(spots)
